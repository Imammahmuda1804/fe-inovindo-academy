...
export default function PaymentPage({ params }) {
  ...
  const batchOptions = useMemo(() => courseData?.batches || [], [courseData]);
  ...
  const handlePayment = React.useCallback(async () => {
    ...
    if (!isReadyToPay || isProcessingPayment) {
      setInlineError(
        "Silakan pilih salah satu opsi yang tersedia untuk melanjutkan."
      );
      return;
    }
    ...
    try {
      ...
      if (summary.totalAmount === 0) {
        ...
        if (response.status === "success") {
          // Cukup periksa status sukses umum
          ...
          showToast(
            response.message ||
              "Gagal mendaftar ke kursus gratis. Silakan coba lagi.",
            "error"
          );
        }
      } else {
        ...
        if (response.status === "success" && response.data.snap_token) {
          ...
          const errorMessage =
            response.message || "Gagal memulai pembayaran. Silakan coba lagi.";
  ...
  const renderContent = () => {
    ...
    const selectedBatch = courseData?.batches?.find(
      (b) => b.id === selectedBatchId
    );
    const selectedPlan = courseData?.pricings?.find(
      (p) => p.id === selectedPlanId
    );
    const selectedPlanName = courseData?.has_batch
      ? selectedBatch?.name
      : selectedPlan?.name;
    const totalStudentsInCourse = courseData?.batches?.reduce(
      (sum, batch) => sum + Number(batch.students_count || 0),
      0
    );
        ...
        <main className="container mx-auto py-8 relative z-10">
          ...
          >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center flex-wrap gap-x-4 gap-y-2 mt-2 text-sm text-gray-600">
                      ...
                      {totalStudentsInCourse > 0 && (
                        <span className="flex items-center gap-1.5">
                          <FaUsers />{" "}
                          {totalStudentsInCourse.toLocaleString("id-ID")} Siswa
                        </span>
                      )}
                    </div>
              ...
              {hasBatches ? (
                ...
                >
                  ...
                  <h2 className="relative z-10 text-xl font-bold text-gray-900 mb-4 flex items-center gap-3">
                    <FaCalendarAlt className="h-6 w-6 text-teal-500" /> Pilih
                    Batch Tersedia
                  </h2>
                  <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6">
                    {batchOptions.map((batch) => {
                      const isFull =
                        ...
                        Number(batch.students_count) >= Number(batch.quota);
                      const filledPercentage =
                        (Number(batch.students_count || 0) /
                          Number(batch.quota || 1)) *
                        100;
                      ...
                      return (
                        <motion.div
                          ...
                          className={`relative p-5 border-2 rounded-2xl transition-all h-full flex flex-col ${
                            isFull || isExpired || isRegistered
                              ...
                              : "cursor-pointer bg-white/60 hover:border-blue-400"
                          } ${
                            selectedBatchId === batch.id
                              ? "border-blue-500 bg-blue-50/80 shadow-xl"
                              : ""
                          }`}
                          whileHover={{
                            scale:
                              isFull || isExpired || isRegistered ? 1 : 1.03,
                          }}
                          onClick={() =>
                            !isFull &&
                            !isExpired &&
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <div
                              className={`flex items-center gap-2 text-xs font-semibold py-1 px-2.5 rounded-full ${
                                isFull
                ...
                >
                  <h2 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-3">
                    <FaGift className="h-6 w-6 text-purple-500" /> Pilih Paket
                    Akses
                  </h2>
                    ...
                    {pricingOptions.map((plan) => (
                      ...
                      >
                        ...
                        <div className="relative z-10 flex flex-col h-full">
                          ...
                          <ul className="space-y-3 text-gray-700 mb-8 grow">
            ...
            <motion.div variants={itemVariants}>
              <div className="overflow-hidden bg-white/60 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl sticky top-24">
                    ...
                    {!isReadyToPay ? (
                      ...
                      >
                        ...
                        <p className="text-sm font-medium">
                          {hasBatches
                            ? "Pilih batch untuk melihat rincian pesanan."
                            : "Pilih paket untuk melihat rincian pesanan."}
                        </p>
                      </motion.div>
                    ) : (
                      <motion.div
                  ...
                  >
                    <button
                      ...
                      className={`w-full bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold text-lg transition-colors duration-300 transform ${
                        isReadyToPay && !isProcessingPayment
                          ? "hover:bg-blue-700"
